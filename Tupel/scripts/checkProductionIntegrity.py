...
#    print 'this is dsetname >>>>>>> %s'% dsetname   
...
#            Popen([eos_cmd, 'mkdir', '/eos/cms/'+newDir],stdout=PIPE).communicate()
            ...
            if len(file_list)>0:
                ...
                if subgroupMerge>0:
                    ...
                    for ilist in xrange(0,len(split_file_lists)):
                        if pubExt:
                            mergedFileName='/tmp/mgul/MergedMiniEvents_%d_%s.root '%(ilist,pubExt)
                            mergedFileNameonly='MergedMiniEvents_%d_%s.root '%(ilist,pubExt)
                        else:
                            mergedFileName='/tmp/mgul/MergedMiniEvents_%d.root '%ilist
                            mergedFileNameonly='MergedMiniEvents_%d.root '%ilist
                        toAdd='%s ' % mergedFileName
                        for f in split_file_lists[ilist]:                            
                            toAdd += '/pnfs/iihe/cms%s '%f 

                        finalOutput='/pnfs/iihe/cms%s/%s'%(newDir,mergedFileName.replace('/tmp/mgul/',''))
                        fIn=ROOT.TFile.Open(finalOutput)
                        try:
                            if fIn or not fIn.IsZombie():
...
#                        os.system('cp %s /pnfs/iihe/cms%s/%s' % (f, newDir,newF) )
            ...
            if not opt.nocheck and opt.cleanup : 
                choice = raw_input('Will remove output directory. [y/n] ?').lower()
                if 'y' in choice: 
                    Popen(['rm', '-r /pnfs/iihe/cms'+dset],stdout=PIPE).communicate()
