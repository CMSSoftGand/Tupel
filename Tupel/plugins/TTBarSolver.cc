...
void TTBarSolver::Solve(Permutation &hyp, bool lazy)
{
  ...
  WTmass_right_->Scale(1./WTmass_right_->Integral("width"));
	...
	if(nschi > 0. && nschi < 10000. && N_right_->GetXaxis()->GetXmin()>=0. && N_right_->GetXaxis()->GetXmax()<=150.)
//	if(nschi > 0. && nschi < 10000. && N_right_)
{
	...
	if(mthad > 0. && mthad < 500. && mwhad > 0. && mwhad < 500. && WTmass_right_) {//for -ve values the extra condition applied but why the -ve values of w and t masses are???????????
