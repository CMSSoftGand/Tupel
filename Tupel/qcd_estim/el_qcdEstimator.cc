    ...
    void el_qcdEstimator(TString fin,TString fout){
...
//        nentries=100000;
      ...
      int n_pat_elec = 0,n_elec20=0,n_el10p15_33=0,n_elp15_33=0;
      std::vector<TLorentzVector>lep_vector, loose_lep_vector, el_vectorp15_33,lep_vectorp15_33, el_vector10p15_33,lep_vector10p15_33;
      TLorentzVector v_lep, v_elp15_33, v_lepp15_33;
      vector<unsigned int> n_elec_v,n_elec20_v, n_lep_v,n_el_vp15_33, n_lep_vp15_33;
      vector<int> lep_charge, el_chargep15_33, n_el10_vp15_33, lep_chargep15_33, n_lep10_vp15_33;
      ...
      bool ip_cut=false, veto_e_cut=false, tight_e_cut=false, antiIso_e_cut=false;
      bool Iso_Rho_loose=false,Iso_Rho_tight=false,Iso_Rho_veto=false, antiIso_Rho=false;
	if (fabs(patElecScEta_->at(elec))<=1.479){
      if(patElecPfIsoRho_->at(elec) < 0.0994)Iso_Rho_loose=true;
      if(patElecPfIsoRho_->at(elec) < 0.0588)Iso_Rho_tight=true;
   //   if(patElecPfIsoRho_->at(elec) >= 0.0588 && patElecPfIsoRho_->at(elec)< 0.083)antiIso_Rho=true;
   //   if(patElecPfIsoRho_->at(elec) >= 0.083 && patElecPfIsoRho_->at(elec) < 0.13)antiIso_Rho=true;
   //   if(patElecPfIsoRho_->at(elec) >= 0.13)antiIso_Rho=true;
      if(patElecPfIsoRho_->at(elec) >= 0.0588 )antiIso_Rho=true;
      if(patElecPfIsoRho_->at(elec) < 0.1750)Iso_Rho_veto=true;
      if (fabs(patElecd0_->at(elec)) < 0.05 && fabs(patElecdz_->at(elec)) < 0.10)ip_cut=true;
      }
	if (fabs(patElecScEta_->at(elec))>1.479 ){
      if(patElecPfIsoRho_->at(elec) < 0.1070)Iso_Rho_loose=true;
      if(patElecPfIsoRho_->at(elec) < 0.0571)Iso_Rho_tight=true;
  //    if(patElecPfIsoRho_->at(elec) >= 0.0571 && patElecPfIsoRho_->at(elec) < 0.083)antiIso_Rho=true;
  //    if(patElecPfIsoRho_->at(elec) >= 0.083 && patElecPfIsoRho_->at(elec) < 0.13)antiIso_Rho=true;
  //    if(patElecPfIsoRho_->at(elec) >= 0.13)antiIso_Rho=true;
      if(patElecPfIsoRho_->at(elec) >= 0.0571)antiIso_Rho=true;
      if(patElecPfIsoRho_->at(elec) < 0.1590)Iso_Rho_veto=true;
      if (fabs(patElecd0_->at(elec)) < 0.10 && fabs(patElecdz_->at(elec)) < 0.20)ip_cut=true;
      }
//cout<<"this is test: "<<fabs(patElecScEta_->at(elec))<<", "<<Iso_Rho_tight<<endl;
        if(patElecPt_->at(elec)>20 && fabs(patElecScEta_->at(elec))<2.5 && passVetoId && Iso_Rho_veto)
           veto_e_cut=true;
        if (patElecPt_->at(elec) > 30. && fabs(patElecScEta_->at(elec)) <2.5 && passTightId && Iso_Rho_tight && 
            (1.4442 > fabs( patElecScEta_->at(elec)) || fabs(patElecScEta_->at(elec))> 1.5660))
            tight_e_cut=true;
        if (patElecPt_->at(elec) > 30. && fabs(patElecScEta_->at(elec)) <2.5 && passTightId && antiIso_Rho &&
          (1.4442 > fabs( patElecScEta_->at(elec)) || fabs(patElecScEta_->at(elec))> 1.5660))
          antiIso_e_cut=true;
          
        if (antiIso_e_cut){
           n_elp15_33++;
           n_el_vp15_33.push_back(n_elp15_33);
           v_elp15_33.SetPtEtaPhiE(patElecPt_->at(elec),patElecEta_->at(elec),patElecPhi_->at(elec),patElecEnergy_->at(elec));
           el_vectorp15_33.push_back(v_elp15_33);
           el_chargep15_33.push_back(patElecCharge_->at(elec));
           }
        if (n_el_vp15_33.size()==0 && veto_e_cut){
           TLorentzVector el10p15_33_vector_;
           el10p15_33_vector_.SetPtEtaPhiE(patElecPt_->at(elec),patElecEta_->at(elec),patElecPhi_->at(elec),patElecEnergy_->at(elec));
           el_vector10p15_33.push_back(el10p15_33_vector_);
           n_el10p15_33++;
           n_el10_vp15_33.push_back(n_el10p15_33);
           }
      ...
      //////-----------------------------------muon------------------------------------------------------------//
      bool loose_mu_cut(false),tight_mu_cut(false);
      ...
      //--- generalize for elecron and muon
        ...
        if (!is_mu){ // for anti-isolated electron
          lep_vector10p15_33=el_vector10p15_33;
          n_lep_vp15_33 = n_el_vp15_33;
          n_lep10_vp15_33 = n_el10_vp15_33;
          lep_vectorp15_33 = el_vectorp15_33;
          lep_chargep15_33 = el_chargep15_33;
          v_lepp15_33=v_elp15_33;
          }
...
//----------------Rochester algorithm----------------
    ...
    double nuDisc=0., massDiscr=0., nuchi2=0.;
    TTBarSolver solver;
